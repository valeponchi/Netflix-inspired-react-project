...
const useStore = create(
	devtools((set, get) => ({
		...
		fetchSearch: input => {
			fetch(
				`https://api.themoviedb.org/3/search/multi?api_key=c0341e4fa2480ab7786c426cfdc94215&language=en-US&query=${input}&page=1&include_adult=false`
			)
				.then(resp => resp.json())
				.then(searchedItems => {
					set({ searchedMovies: searchedItems.results })
				})
		},
	}))
)
